...
// Source: orca/src/renderer/src/components/sidebar/worktree-list/viewport/hard-scroll-up.ts
...
function withVisibility(
  ...
  if (maxScroll < HARD_SCROLL_UP.minScrollablePx || scrollTop <= HARD_SCROLL_UP.nearTopPx) {
    if (
      !state.visible &&
  ...
  if (intent && scrollTop >= HARD_SCROLL_UP.minDepthPx) {
    return {
      ...state,
      visible: true,
      lastIntentAt: t
...
export function reduceHardScrollUpOnWheel(
  ...
  if (maxScroll < HARD_SCROLL_UP.minScrollablePx) {
    return createHardScrollUpDetectorState();
  }

  if (scrollTop <= HARD_SCROLL_UP.nearTopPx) {
