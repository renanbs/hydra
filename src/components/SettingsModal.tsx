...
export function SettingsModal({ isOpen, onClose, onSaved, onLiveChange }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="relative w-[960px] max-w-[95vw] h-[640px] max-h-[92vh] rounded-xl border bg-background shadow-2xl flex overflow-hidden">
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="rounded-xl border bg-card divide-y">
                      <div className="p-4 grid grid-cols-3 gap-4">
                        <div><label className="block text-[12px] font-medium mb-1.5">Cursor Style</label><select value={settings.terminal_cursor_style} onChange={(e)=>setSettingsLive({ ...settings, terminal_cursor_style: e.target.value as any })} className="w-full bg-background border rounded-md px-3 py-2 text-[13px]"><option value="block">Block</option><option value="underline">Underline</option><option value="bar">Bar</option></select></div>
                        ...
                        <div><label className="block text-[12px] font-medium mb-1.5">GPU Acceleration</label><select value={settings.terminal_gpu_acceleration} onChange={(e)=>setSettingsLive({ ...settings, terminal_gpu_acceleration: e.target.value as any })} className="w-full bg-background border rounded-md px-3 py-2 text-[13px]"><option value="auto">Auto</option><option value="on">On</option><option value="off">Off</option></select></div>
                        <div><label className="block text-[12px] font-medium mb-1.5">Ligatures</label><select value={settings.terminal_ligatures} onChange={(e)=>setSettingsLive({ ...settings, terminal_ligatures: e.target.value as any })} className="w-full bg-background border rounded-md px-3 py-2 text-[13px]"><option value="auto">Auto</option><option value="on">On</option><option value="off">Off</option></select></div>
              ...
              {activeId==="agents" && (
                <div className="space-y-8">
                  ...
                  <section className="rounded-xl border bg-card p-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <select value={String((settings as any).default_tui_agent ?? "auto")} onChange={(e)=>{ const v=e.target.value; const agent = v==="auto" ? null : v==="blank" ? "blank" : v; setSettingsLive({ ...settings, default_tui_agent: agent as any }); }} className="flex-1 bg-background border rounded-md px-3 py-2 text-[13px]">
